...
- `GEMINI_LLM_MODEL` (défaut `gemini/gemini-3.5-flash`), `GEMINI_API_KEY`, `GEMINI_FALLBACK_MODELS`.
...
`LLMRouter` supporte désormais 5 rotations via `LLM_ROTATION` :

| Rotation | Comportement |
|---|---|
| `round_robin` (défaut) | alternance fournisseurs/modèles configurés |
| `failover` | premier fournisseur sain, sinon fallback |
| `least_used` | le moins utilisé récemment |
| `random` | shuffle uniforme des fournisseurs **et** des modèles |
| `adaptive` | tirage pondéré par la santé : `0.05 + succès/total` par fournisseur **et** par modèle |
