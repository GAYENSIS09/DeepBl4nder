...
def _make(tmp_path, name: str, content: str) -> Artifact:
    path = tmp_path / f"{name}.png"
    path.write_text(content, encoding="utf-8")
    return Artifact(type="render", name=name, path=path)


def test_register_versions_by_type_and_name(tmp_path) -> None:
...
def test_register_computes_hash(tmp_path) -> None:
    registry = ArtifactRegistry()
...
def test_status_lifecycle(tmp_path) -> None:
    registry = ArtifactRegistry()
