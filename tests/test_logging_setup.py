...
from deepblender.logging_setup import log_file_path, setup_logging
...
def test_setup_logging_writes_background_events_to_file(tmp_path) -> None:
    ...
    logging.getLogger("deepblender.test").info("événement de contrôle %s", 42)
    for handler in root.handlers:
        handler.flush()
    content = path.read_text(encoding="utf-8")
    assert "événement de contrôle 42" in content
    assert "deepblender.test" in content
...
def test_setup_logging_is_idempotent(tmp_path) -> None:
    ...
    before = len(logging.getLogger("deepblender").handlers)
    setup_logging(data_dir=str(data_dir))
    after = len(logging.getLogger("deepblender").handlers)
    assert before == after
